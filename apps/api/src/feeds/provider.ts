...
export function createTheOddsClient(fetchImpl: typeof fetch = fetch): ProviderClient {
  return {
    ...
    async pendingMatches(sportKey: string, apiKey: string): Promise<TheOddsMatch[]> {
      const url = `${BASE}/${encodeURIComponent(sportKey)}/odds/?regions=eu&markets=h2h,spreads,totals&oddsFormat=decimal&dateFormat=iso&includeLinks=false`;
      const res = await fetchImpl(url, { headers: { 'x-apisports-key': '', 'api-key': apiKey, Accept: 'application/json' } });
